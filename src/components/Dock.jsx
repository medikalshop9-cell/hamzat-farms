...
function DockItem({ item, mouseX, baseItemSize, magnification, panelHeight }) {
  ...
  const widthPx = useSpring(
    useTransform(distance, [0, 150], [magnification, baseItemSize]),
  ...
  return (
    ...
    >
      <span style={{ color: "#fff", fontSize: 18 }}>{item.icon}</span>
      {/* Tooltip */}
      <motion.span
